...
const putContractOperation = async (type: any) => {
  state.loading = true
  const res: any = await contractOperation({
...
<template>
  ...
  <dl v-if="state.item">
    ...
    <dd v-if="store.role == 3">
      ...
      <div class="small-item-text">
        ...
        <p>{{ state.item.position_name }} ｜{{ state.item.sex }} ｜ {{ state.item.work_year }} ｜ {{
  ...
  <div class="contract-btn" v-if="state.item">
    ...
    <button class="confirm-btn" @click="gotoProgress"
      v-if="state.item.is_contract_type != 1 && state.item.is_contract_type != 2">合约进度</button>
  </div>
