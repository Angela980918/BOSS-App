...
<template>
  ...
  <div class="position-type">
    <div class="position-type-left">
      <h5 :class="state.typekey == index ? 'active' : ''" v-for="(item, index) in store.positionList" :key="index"
        @click="setTypeKey(index)">{{ item.name }}</h5>
    </div>
    <div class="position-type-right">
      <span v-for="(item, index) in store.positionList[state.typekey] && store.positionList[state.typekey].children"
        :key="index" @click="closePositionType(item.name)">{{ item.name }}</span>
    </div>
  </div>
</template>
