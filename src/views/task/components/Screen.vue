...
<template>
  ...
  <div class="task-screen">
    ...
    <div class="screen-item">
      <span :class="state.mode == item ? 'active' : ''" v-for="(item, index) in store.screenList.serviceMode" :key="index"
        @click="modeChange(item)">{{ item }}</span>
    </div>
    <h3>任务周期</h3>
    <div class="screen-item">
      <span :class="state.cycle == item ? 'active' : ''" v-for="(item, index) in store.screenList.taskCycle" :key="index"
        @click="cycleChange(item)">{{ item }}</span>
    </div>
  </div>
